    ...
    if uploaded_file:
        ...
        try:
            ...
            with col2:
                ...
                st.markdown("*Bacterium exposed multiple times*")
                st.warning("⚠️ Resistance likelihood increases significantly!")
                for antibiotic_class in ['beta_lactam', 'aminoglycoside', 'fluoroquinolone']:
                    evolution = predict_resistance_evolution(seq, antibiotic_class)
            ...
            if risk_data:
                ...
                st.success("✓ No major genomic risk factors detected.")
            
            st.subheader("🧬 Likely Resistance Genes Under Pressure")
            if len(models) > 0 and amr_genes_db:
                selected_antibiotic = st.selectbox("Select antibiotic for gene prediction:", list(amr_genes_db.keys()))
                if selected_antibiotic in amr_genes_db:
